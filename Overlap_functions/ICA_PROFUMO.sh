...
fsl_sub -l logfiles -N M_${Option} \
    melodic -i ${homedir}/Results/input_filelist_${Option}.txt  \
    -o ${homedir}/Results/Melodic_${Option}.gica \
    --tr=${tr} --nobet --nomask -a concat --disableMigp -d ${dim}

# run PROFUMO (need to make sure we're on jalapeno18)
nice -n 20 ~samh/bin/PROFUMO \
    ${homedir}/Results/PFMsims_atlas_${Option}.json \
    ${dim} \
    ${homedir}/Results/PROFUMO_PFMsims_atlas_${Option} \
    --useHRF ${tr} --hrfFile ~samh/PROFUMO/Scripts/DefaultHRF.phrf \
    --nThreads 15 -d 0.5 > ${homedir}/Results/Output_PROFUMO_PFMsims_atlas_${Option}.txt
