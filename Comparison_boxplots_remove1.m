...
% Inputs to compare
Ins = {'original_ThursdayAfternoon','LowO_LowM_HighT_ThursdayAfternoon','HighO_LowM_LowT_ThursdayAfternoon','LowO_HighM_LowT_ThursdayAfternoon'};
Ins = {'NoO_NoM_HighT400_01'};
%Ins = {'LowO_LowM_HighT_01','LowO_LowM_HighT_02','LowO_LowM_HighT_03','LowO_LowM_HighT_04'};
save_figs = 1;
...
Ai = ones(params.iN-1); Ai = triu(Ai,1); Ai = find(Ai); An = length(Ai);
REM = 2; 
...
Netmats = zeros(params.iN-1,(params.iN-1)*2,5,length(Ins));
Snets = zeros(params.S,4,length(Ins));
Tnets = zeros(params.S,8,length(Ins));
Scorrs = zeros(params.S,params.iN-1,4,length(Ins));
Tcorrs = zeros(params.S,params.iN-1,8,length(Ins));
...
for n = 1:length(Ins)
    ...
    % Get group netmats
    Ntemp = zeros(params.iN-1,params.iN-1,params.S,5);
    ...
    for s = 1:params.S
        ...
        % Ground truth nets
        [r,~] = spatialcorr(P{s},P{s}); r(REM,:) = []; r(:,REM) = []; SG = r(Ai);
        r = corr([A{s}{1}'; A{s}{2}']); r(REM,:) = []; r(:,REM) = []; Ntemp(:,:,s,1) = r; TG = r(Ai); 
        TSG = [A{s}{1}'; A{s}{2}'];
        
        % PFM nets
        p = pfmP1_new{s}; [r,~] = spatialcorr(p(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',atlasParams.V,1), p(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',atlasParams.V,1)); r(REM,:) = []; r(:,REM) = []; 
        Snets(s,1,n) = corr(r(Ai),SG);
        [r,~] = spatialcorr(p(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',atlasParams.V,1),P{s}); r(REM,:) = []; r(:,REM) = []; 
        Scorrs(s,:,1,n) = r(eye(params.iN-1)==1)'; 
        % PFM clean ts
            a = [pfmA1_new{s}{1}'; pfmA1_new{s}{2}']; 
            r = corr(a(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
            Tcorrs(s,:,1,n) = r(eye(params.iN-1)==1); 
            r = corr(a(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
        ...
        % PFM DR TS
            ...
            r = corr(a(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
            Tcorrs(s,:,3,n) = r(eye(params.iN-1)==1); 
            r = corr(a(:,Orders(:,2,n)).*repmat(Signs(:,2,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
            Tnets(s,3,n) = corr(r(Ai),TG); 
            clear p a
        
        % tICA nets
        p = tica_P1_DRnew{s}; [r,~] = spatialcorr(p(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',atlasParams.V,1), p(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',atlasParams.V,1)); r(REM,:) = []; r(:,REM) = []; 
        Snets(s,2,n) = corr(r(Ai),SG);
        [r,~] = spatialcorr(p(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',atlasParams.V,1),P{s}); r(REM,:) = []; r(:,REM) = []; 
        Scorrs(s,:,2,n) = r(eye(params.iN-1)==1); clear p
        a = [tica_A1_DRnew{s}{1}; tica_A1_DRnew{s}{2}]; 
        r = corr(a(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
        Tcorrs(s,:,4,n) = r(eye(params.iN-1)==1); 
        r = corr(a(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
        Ntemp(:,:,s,3) = r; 
        Tnets(s,4,n) = corr(r(Ai),TG); clear a
        
        a = [ticaA1{s}{1}'; ticaA1{s}{2}']; 
        r = corr(a(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
        Tcorrs(s,:,5,n) = r(eye(params.iN-1)==1); 
        r = corr(a(:,Orders(:,3,n)).*repmat(Signs(:,3,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
        Tnets(s,5,n) = corr(r(Ai),TG); clear a
        
        % mICA nets
        p = mica_P1_DRnew{s}; [r,~] = spatialcorr(p(:,Orders(:,4,n)).*repmat(Signs(:,4,n)',atlasParams.V,1), p(:,Orders(:,4,n)).*repmat(Signs(:,4,n)',atlasParams.V,1)); r(REM,:) = []; r(:,REM) = []; 
        Snets(s,3,n) = corr(r(Ai),SG);
        [r,~] = spatialcorr(p(:,Orders(:,4,n)).*repmat(Signs(:,4,n)',atlasParams.V,1),P{s}); r(REM,:) = []; r(:,REM) = []; 
        Scorrs(s,:,3,n) = r(eye(params.iN-1)==1); clear p
        a = [micaA1{s}{1}'; micaA1{s}{2}']; 
        r = corr(a(:,Orders(:,4,n)).*repmat(Signs(:,4,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
        Tcorrs(s,:,6,n) = r(eye(params.iN-1)==1); 
        r = corr(a(:,Orders(:,4,n)).*repmat(Signs(:,4,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
        Ntemp(:,:,s,4) = r; 
        ...
        % sICA original spatial measures
            [r,~] = spatialcorr(p(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',atlasParams.V,1), p(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',atlasParams.V,1)); r(REM,:) = []; r(:,REM) = []; 
            Snets(s,4,n) = corr(r(Ai),SG); 
            [r,~] = spatialcorr(p(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',atlasParams.V,1),P{s}); r(REM,:) = []; r(:,REM) = []; 
            Scorrs(s,:,4,n) = r(eye(params.iN-1)==1);
        % sICA original temporal measures         
            a = [sica_A1_DR_new{s}{1}; sica_A1_DR_new{s}{2}]; 
            r = corr(a(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
            Tcorrs(s,:,7,n) = r(eye(params.iN-1)==1); 
            r = corr(a(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
            Ntemp(:,:,s,5) = r; 
            Tnets(s,7,n) = corr(r(Ai),TG); clear a
        % sICA thresholded temporal measures         
            a = [sica_A1_DR_thres{s}{1}; sica_A1_DR_thres{s}{2}]; 
            r = corr(a(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',params.T*2,1),TSG); r(REM,:) = []; r(:,REM) = []; 
            Tcorrs(s,:,8,n) = r(eye(params.iN-1)==1); 
            r = corr(a(:,Orders(:,5,n)).*repmat(Signs(:,5,n)',params.T*2,1)); r(REM,:) = []; r(:,REM) = []; 
            Tnets(s,8,n) = corr(r(Ai),TG); clear a
    end
    Netmats(:,1:params.iN-1,:,n) = squeeze(mean(Ntemp,3));
    Netmats(:,params.iN:end,:,n) = squeeze(Ntemp(:,:,1,:));
...
figure; set(gcf,'Position',[0 570 1900 955],'PaperPositionMode','auto')
for n = 1:length(Ins)
    subplot(length(Ins),5,(n-1)*5+1); imagesc(Netmats(:,:,1,n),[-0.5 0.5]); hold on; vline(params.iN+0.5,'k');
    if n == 1; title('Ground truth full netmats','color','k'); end
    ylabel(sprintf('%s',Ins{n}(1:15)),'interpreter','none')
    xlabel('group mean     example subject'); set(gca,'xtick',[])
    
    subplot(length(Ins),5,(n-1)*5+2); imagesc(Netmats(:,:,2,n),[-0.5 0.5]); hold on; vline(params.iN+0.5,'k');
    if n == 1; title(sprintf('%s full netmats',Mnames{1}),'color','b'); end
    xlabel('group mean     example subject'); set(gca,'xtick',[])
    
    subplot(length(Ins),5,(n-1)*5+3); imagesc(Netmats(:,:,3,n),[-0.5 0.5]); hold on; vline(params.iN+0.5,'k');
    if n == 1; title(sprintf('%s full netmats',Mnames{2}),'color','r'); end
    xlabel('group mean     example subject'); set(gca,'xtick',[])
    
    subplot(length(Ins),5,(n-1)*5+4); imagesc(Netmats(:,:,4,n),[-0.5 0.5]); hold on; vline(params.iN+0.5,'k');
    if n == 1; title(sprintf('%s full netmats',Mnames{3}),'color','g'); end
    xlabel('group mean     example subject'); set(gca,'xtick',[])
    
    subplot(length(Ins),5,(n-1)*5+5); imagesc(Netmats(:,:,5,n),[-0.5 0.5]); hold on; vline(params.iN+0.5,'k');
...
D = reshape(Scorrs,params.S*(params.iN-1),4*length(Ins));
figure; set(gcf,'Position',[0 0 1800 500],'PaperPositionMode','auto')
subplot('Position',[0.07 0.25 0.86 0.65])
boxplot(D,'Labels',MN,'color',Mcmap); XYrotalabel(60,0)
...
D = reshape(Tcorrs,params.S*(params.iN-1),8*length(Ins));
figure; set(gcf,'Position',[0 0 1800 500],'PaperPositionMode','auto')
subplot('Position',[0.07 0.25 0.86 0.65])
boxplot(D,'Labels',TN,'color',Tcmap); XYrotalabel(60,0)
