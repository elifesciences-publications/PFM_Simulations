...
% Inputs
fileName = 'NoO_NoM_HighT400_20PFMs';
No_overlap = false; 
...
for n = 1:params.nRepeats
    ...
    Psimple = laprnd(atlasParams.V,params.N,0,0.5);
    for s = 1:params.N
        Psimple(500*s:500*s+400,s) = Psimple(500*s:500*s+400,s)+rand(401,1)*10+2;
    end
...
%     
    ...
    %% Save results
    clear Ds n pcaA pcaP pcaS pcaU pcaV S1 S2 S12 svdS svdU svdV Y1 Y2 A1 A12 A2
